...
test.describe('Acceptance Tests: Authentication, Dashboard & DTU Creation', () => {
  test('successfully logs in, displays dashboard, and manages system-defined DTUs', async ({ page }) => {
    ...
    // The 5-up row's Yield card surfaces the kWh headline; Peak
    // Power uses the `N W` (whole-number) form. The seed includes a
    // fresh reading for the multi-MPPT Garage Array so the Total
    // view reflects live production rather than always 0; just
    // assert the format.
    await expect(page.locator('#stat-yield-kwh')).toContainText(/\d/, { timeout: 10000 });
    await expect(page.locator('#stat-peak-watts')).toContainText(/\d+ W/, { timeout: 10000 });
